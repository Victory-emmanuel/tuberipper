...
export async function POST(req: NextRequest) {
  try {
    ...
    const videoFormat = videoInfo.formats.find((format) => format.hasVideo && format.height === parseInt(videoResolution, 10));

    const audioFormat = videoInfo.formats.find((format) => format.hasAudio && format.audioQuality === audioQuality
    );
