  ...
  try {
    ...
    const videoFormat = videoInfo.formats.find(
      (format) =>
        format.hasVideo &&
        !format.hasAudio &&
        format.height === parseInt(videoResolution, 10)
    );
    ...
    const audioFormat = videoInfo.formats.find(
      (format) =>
        !format.hasVideo &&
        format.hasAudio &&
        format.audioQuality === audioQuality
    );

    if (!videoFormat || !audioFormat) {
      return NextResponse.json({ error: 'Could not find suitable video/audio formats' }, { status: 400 });
    }
